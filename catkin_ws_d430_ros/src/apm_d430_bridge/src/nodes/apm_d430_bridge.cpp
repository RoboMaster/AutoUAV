...
APM_D430_Bridge::APM_D430_Bridge(const ros::NodeHandle& nh)
    : nh_(nh) {

  // initialize subscribers
  output.header.frame_id = "orbbec_odom";
  output.child_frame_id = "orbbec_camera";
  pose_sub_ = nh_.subscribe("/d430_camera/pose", 10, &APM_D430_Bridge::poseCallback, this);
  // publishers
  // mavros_odom_pub_ =
  ...
  worker_ = std::thread(&APM_D430_Bridge::publishSystemStatus, this);


};

APM_D430_Bridge::~APM_D430_Bridge() { }


void APM_D430_Bridge::poseCallback(const geometry_msgs::PoseStamped& msg) {
...
void APM_D430_Bridge::publishSystemStatus(){
